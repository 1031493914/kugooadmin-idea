package com.mvcweb.interceptor;
...
public class LoginInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		System.out.println("Login is trying.");
		String servletPath = request.getServletPath();
		System.out.println(servletPath);
		
		String[] path = { "/loginSubmit","/loginout"};
		//所有未登录请求
		if (session.getAttribute("sessionAdminUser") == null) {
			for (String pa : path) {
				if (servletPath.equals(pa)) {
					return super.preHandle(request, response, handler);
				}
			}
			System.out.println("Login failed.");
			response.sendRedirect(ProjectSetting.BASE_PATH + "login.jsp");
			return false;
		}
